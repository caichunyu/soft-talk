  ...
  <div class="allDynamic-main">
    <div style="margin-top: .3rem" v-for="(item, index) in getAllDynamicData" :key="item.index" :index="index">
      <el-card shadow="hover" :body-style="{padding:'0.6rem'}">
        <!--        <div class="dynamic"  @click="toChatRecordDetail(item.id)">-->
        <!--          <div class="dynamic-top">-->
        <!--            &lt;!&ndash;          <img class="user-icon" src="../../assets/bg.jpg">&ndash;&gt;-->
        ...
        <!--          </div>-->
        <div v-if="item.type !== 3" class="dynamic-top dynamic-comment">
          <img class="user-icon" src="../../assets/user.svg">
          <b> {{ item.nickName }}: </b>
          <div>{{ item.content }}</div>
          <div style="padding-left: .3rem">{{ item.sendDate }}</div>
        </div>
...
<!--&lt;!&ndash;        <div v-if="item.type !== 3" class="chat-div-div">{{ item.content }}</div>&ndash;&gt;-->
<!--        <div v-if="item.type === 3" style="width: 32rem;margin-left: .3rem">-->
<!--          <img class="user-icon" src="../../assets/user.svg">-->
<!--          <b> {{ item.nickName }}: </b>-->
<!--          <img class="user-icon" style="width: 3rem;height: 3rem;border-radius:0" :src=item.content>-->
